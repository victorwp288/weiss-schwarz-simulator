    fn step_sample_from_logits_with_logp_into_i16_legal_ids<'py>(
        ...
        let mut action_logp_arr = array_mut(py, &action_logp)?;
        let action_logp_slice = action_logp_arr.as_slice_mut().ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>("action_logp not contiguous")
        })?;
    ...
    fn step_sample_from_logits_with_logp_into_i16_legal_ids_nometa<'py>(
        &mut self,
        py: Python<'py>,
        logits: PyReadonlyArray2<f32>,
        seeds: PyReadonlyArray1<u64>,
