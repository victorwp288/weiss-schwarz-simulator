# IMPALA + V-trace tutorial (actor/learner)

This tutorial shows a minimal IMPALA-style actor/learner loop with **V-trace** off-policy correction using `weiss_sim` and PyTorch.

Key takeaways:

- actors may run a stale **behavior** policy; the learner updates a **target** policy
- V-trace corrects the policy-gradient update under off-policy rollouts
- legality masking must be applied consistently for both behavior and target policies

## Architecture sketch

```mermaid
flowchart LR
  A["Actors\n(behavior policy)"] --> B["Unrolls\n(T×N)"]
  B --> C["Learner\n(target policy + value)"]
  C --> D["V-trace\n(v_s targets + advantages)"]
  D --> E["Update learner"]
  E --> F["Sync actor weights\n(periodic)"]
  F --> A
```
...
```bash
python python/examples/impala_vtrace.py --num-envs 32 --unroll-len 32 --updates 200 --enable-shaping
```

## Reference implementation

<!-- GENERATED:IMPALA_VTRACE_SCRIPT:START -->
```python
# python/examples/impala_vtrace.py
...
if __name__ == "__main__":
    main()
```
<!-- GENERATED:IMPALA_VTRACE_SCRIPT:END -->
