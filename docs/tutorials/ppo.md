# PPO tutorial (masked discrete actions)

This tutorial shows a minimal PPO loop for a **masked discrete** action space using `weiss_sim` and PyTorch.

Key takeaways:

- configure `legal_repr="mask_u8"` so legality is available as a dense mask
- apply legality masking to logits (use `-1e9` for illegal actions) before sampling
- reset envs using `step.needs_reset` (covers `done` and engine faults)

This tutorial is algorithm-focused; the default reward is sparse. For easier debugging, enable shaping rewards (see “Reward shaping” below).

## How legality masking works

At each decision boundary, the environment returns `batch.legal.mask` (shape `(N, ACTION_SPACE_SIZE)`).

For masked policies:

1. compute logits from observations
2. set logits for illegal actions to a large negative value (`-1e9`)
3. sample from `Categorical(logits=masked_logits)`

## PPO loop sketch

```mermaid
flowchart LR
  A["Collect rollout\n(T steps × N envs)"] --> B["Compute GAE(λ)\nadvantages + returns"]
  B --> C["Optimize PPO objective\n(K epochs, minibatches)"]
  C --> A
```

## Reward shaping (optional)

By default, non-terminal rewards are `0.0` unless shaping is enabled. You can pass a `reward_json` that matches `RewardConfig` in Rust:

- `enable_shaping` (bool)
- `damage_reward` (float)
- `terminal_win`, `terminal_loss`, `terminal_draw` (floats; keep win/loss zero-sum)

See [Reward semantics](../rl_contract.md#reward-semantics) for details.

## Running the example

```bash
python python/examples/ppo_torch.py --num-envs 32 --rollout-len 128 --updates 50 --enable-shaping
```

## Reference implementation

<!-- GENERATED:PPO_TORCH_SCRIPT:START -->
```python
# python/examples/ppo_torch.py
...
if __name__ == "__main__":
    main()
```
<!-- GENERATED:PPO_TORCH_SCRIPT:END -->

## Next

- [IMPALA + V-trace tutorial](impala_vtrace.md)
